...
#include "ORB_SLAM2_CUBA/include/LoopClosing.h"
#include "ORB_SLAM2_CUBA/include/Optimizer.h"
#include "ORB_SLAM2_CUBA/include/Converter.h"
#include "ORB_SLAM2_CUBA/include/KeyFrame.h"
...
namespace SLAM_Benchmark
{
    ...
    {
        class LoopClosing : public ORB_SLAM2_CUBA::LoopClosing
        {
        public:
            LoopClosing(ORB_SLAM2_CUBA::Map *pMap, ORB_SLAM2_CUBA::KeyFrameDatabase *pDB, ORB_SLAM2_CUBA::ORBVocabulary *pVoc, const bool bFixScale) : ORB_SLAM2_CUBA::LoopClosing(pMap, pDB, pVoc, bFixScale) {}

            // 回环线程主函数
            void Run() override
            {
                SLAM_Benchmark::ThreadRecorder *thread_recorder = SLAM_Benchmark::SystemRecorder::getInstance(SLAM_Benchmark::SystemName::ORB_SLAM2_CUBA)->getThreadRecorder("LoopClosing");
            ...
            void CorrectLoop() override
            {
                ...
                // 对地图点操作
                {
                    ...
                    // 遍历"当前关键帧组""
                    for (vector<ORB_SLAM2_CUBA::KeyFrame *>::iterator vit = mvpCurrentConnectedKFs.begin(), vend = mvpCurrentConnectedKFs.end(); vit != vend; vit++)
                    {
                        ORB_SLAM2_CUBA::KeyFrame *pKFi = *vit;
                        ...
                        if (pKFi != mpCurrentKF) //跳过当前关键帧，因为当前关键帧的位姿已经在前面优化过了，在这里是参考基准
                        {
                            ...
                            g2o::Sim3 g2oSic(ORB_SLAM2_CUBA::Converter::toMatrix3d(Ric), ORB_SLAM2_CUBA::Converter::toVector3d(tic), 1.0);
                        ...
                        g2o::Sim3 g2oSiw(ORB_SLAM2_CUBA::Converter::toMatrix3d(Riw), ORB_SLAM2_CUBA::Converter::toVector3d(tiw), 1.0);
                    ...
                    {
                        // 取出当前关键帧连接关键帧
                        ORB_SLAM2_CUBA::KeyFrame *pKFi = mit->first;
                        // 取出经过位姿传播后的Sim3变换
                        ...
                        vector<ORB_SLAM2_CUBA::MapPoint *> vpMPsi = pKFi->GetMapPointMatches();
                        // 遍历待矫正共视关键帧中的每一个地图点
                        for (size_t iMP = 0, endMPi = vpMPsi.size(); iMP < endMPi; iMP++)
                        {
                            ORB_SLAM2_CUBA::MapPoint *pMPi = vpMPsi[iMP];
                            // 跳过无效的地图点
                            if (!pMPi)
                                continue;
                            if (pMPi->isBad())
                            ...
                            // 地图点世界坐标系下坐标
                            Eigen::Matrix<double, 3, 1> eigP3Dw = ORB_SLAM2_CUBA::Converter::toVector3d(P3Dw);
                            // map(P) 内部做了相似变换 s*R*P +t
                            // 下面变换是：eigP3Dw： world →g2oSiw→ i →g2oCorrectedSwi→ world
                            Eigen::Matrix<double, 3, 1> eigCorrectedP3Dw = g2oCorrectedSwi.map(g2oSiw.map(eigP3Dw));

                            cv::Mat cvCorrectedP3Dw = ORB_SLAM2_CUBA::Converter::toCvMat(eigCorrectedP3Dw);
                        ...
                        cv::Mat correctedTiw = ORB_SLAM2_CUBA::Converter::toCvSE3(eigR, eigt);
                        // 设置矫正后的新的pose
                    ...
                    for (size_t i = 0; i < mvpCurrentMatchedPoints.size(); i++)
                    {
                        if (mvpCurrentMatchedPoints[i])
                        {
                            ...
                            // 匹配投影得到的地图点
                            ORB_SLAM2_CUBA::MapPoint *pLoopMP = mvpCurrentMatchedPoints[i];
                            // 原来的地图点
                            ORB_SLAM2_CUBA::MapPoint *pCurMP = mpCurrentKF->GetMapPoint(i);
                            if (pCurMP)
                ...
                map<ORB_SLAM2_CUBA::KeyFrame *, set<ORB_SLAM2_CUBA::KeyFrame *>> LoopConnections;
                ...
                for (vector<ORB_SLAM2_CUBA::KeyFrame *>::iterator vit = mvpCurrentConnectedKFs.begin(), vend = mvpCurrentConnectedKFs.end(); vit != vend; vit++)
                {
                    ...
                    vector<ORB_SLAM2_CUBA::KeyFrame *> vpPreviousNeighbors = pKFi->GetVectorCovisibleKeyFrames();
                    ...
                    for (vector<ORB_SLAM2_CUBA::KeyFrame *>::iterator vit_prev = vpPreviousNeighbors.begin(), vend_prev = vpPreviousNeighbors.end(); vit_prev != vend_prev; vit_prev++)
                    {
                        LoopConnections[pKFi].erase(*vit_prev);
                    }
                    // Step 5.6：从连接关系中去除闭环之前的一级连接关系，剩下的连接就是由闭环得到的连接关系
                    for (vector<ORB_SLAM2_CUBA::KeyFrame *>::iterator vit2 = mvpCurrentConnectedKFs.begin(), vend2 = mvpCurrentConnectedKFs.end(); vit2 != vend2; vit2++)
                    {
                ...
                ORB_SLAM2_CUBA::Optimizer::OptimizeEssentialGraph(mpMap, mpMatchedKF, mpCurrentKF, NonCorrectedSim3, CorrectedSim3, LoopConnections, mbFixScale);
            ...
            void RunGlobalBundleAdjustment(unsigned long nLoopKF)
            {
                ...
                ORB_SLAM2_CUBA::Optimizer::GlobalBundleAdjustemnt(mpMap,      // 地图点对象
                                                             10,         // 迭代次数
                ...
                // 所以和更新后的地图并不连续。需要通过spanning tree来传播
                {
                    ...
                    if (!mbStopGBA)
                    {
                        ...
                        list<ORB_SLAM2_CUBA::KeyFrame *> lpKFtoCheck(mpMap->mvpKeyFrameOrigins.begin(), mpMap->mvpKeyFrameOrigins.end());
                        ...
                        {
                            ORB_SLAM2_CUBA::KeyFrame *pKF = lpKFtoCheck.front();
                            const set<ORB_SLAM2_CUBA::KeyFrame *> sChilds = pKF->GetChilds();
                            cv::Mat Twc = pKF->GetPoseInverse();
                            // 遍历当前关键帧的子关键帧
                            for (set<ORB_SLAM2_CUBA::KeyFrame *>::const_iterator sit = sChilds.begin(); sit != sChilds.end(); sit++)
                            {
                                ORB_SLAM2_CUBA::KeyFrame *pChild = *sit;
                                // 记录避免重复
                        ...
                        // Correct MapPoints
                        const vector<ORB_SLAM2_CUBA::MapPoint *> vpMPs = mpMap->GetAllMapPoints();

                        // Step 3 遍历每一个地图点并用更新的关键帧位姿来更新地图点位置
                        for (size_t i = 0; i < vpMPs.size(); i++)
                        {
                            ORB_SLAM2_CUBA::MapPoint *pMP = vpMPs[i];
                            ...
                            else
                            {
                                ...
                                ORB_SLAM2_CUBA::KeyFrame *pRefKF = pMP->GetReferenceKeyFrame();
