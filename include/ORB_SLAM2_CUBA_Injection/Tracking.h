...
#include "ORB_SLAM2_CUBA/include/Tracking.h"
...
namespace SLAM_Benchmark
{
    ...
    {
        class Tracking : public ORB_SLAM2_CUBA::Tracking
        {
        public:
            Tracking(ORB_SLAM2_CUBA::System *pSys, ORB_SLAM2_CUBA::ORBVocabulary *pVoc, ORB_SLAM2_CUBA::FrameDrawer *pFrameDrawer,
                     ORB_SLAM2_CUBA::MapDrawer *pMapDrawer, ORB_SLAM2_CUBA::Map *pMap, ORB_SLAM2_CUBA::KeyFrameDatabase *pKFDB,
                     const string &strSettingPath, const int sensor)
                : ORB_SLAM2_CUBA::Tracking(pSys, pVoc, pFrameDrawer, pMapDrawer, pMap, pKFDB, strSettingPath, sensor) {}
            ...
            {
                SLAM_Benchmark::ThreadRecorder *thread_recorder = SLAM_Benchmark::SystemRecorder::getInstance(SLAM_Benchmark::SystemName::ORB_SLAM2_CUBA)->getThreadRecorder("Tracking");
                ...
                if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET) //没有成功初始化的前一个状态就是NO_IMAGES_YET
                    mCurrentFrame = ORB_SLAM2_CUBA::Frame(
                        mImGray,
                        timestamp,
            ...
            */
            void Track() override
            {
                ...
                {
                    if (mSensor == ORB_SLAM2_CUBA::System::STEREO || mSensor == ORB_SLAM2_CUBA::System::RGBD)
                        //双目RGBD相机的初始化共用一个函数
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                //运动模型中构造的地图点
                                vector<ORB_SLAM2_CUBA::MapPoint *> vpMPsMM;
                                //在追踪运动模型后发现的外点
                                vector<bool> vbOutMM;
                                //运动模型得到的位姿
                                cv::Mat TcwMM;
                    ...
                    if (bOK)
                    {
                        ...
                        for (int i = 0; i < mCurrentFrame.N; i++)
                        {
                            ORB_SLAM2_CUBA::MapPoint *pMP = mCurrentFrame.mvpMapPoints[i];
                            if (pMP)
                                if (pMP->Observations() < 1)
                                {
                                    mCurrentFrame.mvbOutlier[i] = false;
                                    mCurrentFrame.mvpMapPoints[i] = static_cast<ORB_SLAM2_CUBA::MapPoint *>(NULL);
                                }
                        }
                        ...
                        for (list<ORB_SLAM2_CUBA::MapPoint *>::iterator lit = mlpTemporalPoints.begin(), lend = mlpTemporalPoints.end(); lit != lend; lit++)
                        {
                            ORB_SLAM2_CUBA::MapPoint *pMP = *lit;
                            delete pMP;
                        }
                    ...
                    // 保存上一帧的数据,当前帧变上一帧
                    mLastFrame = ORB_SLAM2_CUBA::Frame(mCurrentFrame);
                }
