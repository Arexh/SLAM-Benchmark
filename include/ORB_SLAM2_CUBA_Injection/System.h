...
#include "ORB_SLAM2_CUBA/include/System.h"
#include <unistd.h>
#include <iostream>
#include <future>
#include <thread>
...
#include "ORB_SLAM2_CUBA_Injection/Tracking.h"

namespace SLAM_Benchmark
{
    namespace ORB_SLAM2_CUBA_Inject
    {
        class System : public ORB_SLAM2_CUBA::System
        {
        public:
                   ...
                   const bool bUseViewer = true)
            {
                /* init members */
                mSensor = sensor;
                mpViewer = static_cast<ORB_SLAM2_CUBA::Viewer *>(NULL);
                mbReset = false;
                ...
                //建立一个新的ORB字典
                mpVocabulary = new ORB_SLAM2_CUBA::ORBVocabulary();
                //获取字典加载状态
                ...
                mpKeyFrameDatabase = new ORB_SLAM2_CUBA::KeyFrameDatabase(*mpVocabulary);

                //Create the Map
                mpMap = new ORB_SLAM2_CUBA::Map();

                //Create Drawers. These are used by the Viewer
                //这里的帧绘制器和地图绘制器将会被可视化的Viewer所使用
                mpFrameDrawer = new ORB_SLAM2_CUBA::FrameDrawer(mpMap);
                mpMapDrawer = new ORB_SLAM2_CUBA::MapDrawer(mpMap, strSettingsFile);
                ...
                if (bUseViewer)
                {
                    ...
                    //新建viewer
                    mpViewer = new ORB_SLAM2_CUBA::Viewer(this,             //又是这个
                                                     mpFrameDrawer,    //帧绘制器
                                                     mpMapDrawer,      //地图绘制器
                                                     mpTracker,        //追踪器
                                                     strSettingsFile); //配置文件的访问路径
                    //新建viewer线程
                    mptViewer = new thread(&ORB_SLAM2_CUBA::Viewer::Run, mpViewer);
                    //给运动追踪器设置其查看器
